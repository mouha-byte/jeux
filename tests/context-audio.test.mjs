...
const source=transformSync(readFileSync(new URL('../lib/context-audio.ts',import.meta.url),'utf8'),{loader:'ts',format:'cjs'}).code;
...
test('context sounds: each profile renders distinct valid envelopes with no media fetch',async()=>{
 const signatures=[];
 for(const [profile,event] of [['chess','move'],['pool','collision'],['xo','placeX'],['cards','deal'],['rps','count']]){const {Audio,trace}=harness(),audio=new Audio(profile);await audio.play(event);assert.ok(trace.some(e=>e[0]==='start'));signatures.push(JSON.stringify(trace));audio.stop()}
 assert.equal(new Set(signatures).size,5);
 for(const profile of ['chess','pool','xo','cards','rps']){const {Audio,trace}=harness();const audio=new Audio(profile);await audio.play('win');assert.ok(trace.filter(e=>e[0]==='start').length>=4);audio.stop()}
});
...
test('other games no longer import or request Ludo audio',()=>{
 for(const name of ['app/pool/page.tsx','app/xo/page.tsx','app/uno/page.tsx','app/rps/page.tsx','lib/chess-sound.ts']){const text=readFileSync(new URL('../'+name,import.meta.url),'utf8');assert.equal(/GameAudio|game-audio|tokenMove|tokenKill|tokenHome|btnSound/.test(text),false,name)}
});
